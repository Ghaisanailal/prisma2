...
if (move_uploaded_file($_FILES['file']['tmp_name'], $uploadDir . $rand . '_' . $filename)) {
    ...
    // Menjalankan query UPDATE berdasarkan idpensiun
    $query = "UPDATE pensiun SET status = '$status', ket = '$ket', sk = '$filenameSk' WHERE idpensiun = '$idpensiun'";
    ...
    if (!$result) {
        ...
        echo "<script>window.location='?page=pensiun-tampil&alert=3';</script>";
    }
} else {
